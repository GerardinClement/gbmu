            ...
            .show(ctx, |ui| {
                eframe::egui::ScrollArea::vertical().show(ui, |ui| {
                    ...
                    ui.group(|ui| {
                        ...
                        step_mode_button(ui, &mut self.emulated_game);
                        step_button(ui, &mut self.emulated_game);
                    });
                            ...
                            ui.horizontal(|ui| {
                                ...
                                ui.add(
                                    eframe::egui::DragValue::new(
                                        &mut self.emulated_game.nb_instruction,
                                    )
                                    .speed(1.0)
                                    .clamp_range(0..=255)
                                    .prefix("Dec: "),
                                ...
                                // Fetch button
                                let fetch_btn = ui.add_sized(
                                    [100.0, 20.0],
                                    eframe::egui::Button::new("📋 Fetch"),
                                );

                                if fetch_btn.clicked() {
                                    self.emulated_game
                                        .get_next_instructions(self.emulated_game.nb_instruction);
                                }
                            });
                        });
                    });
                    ...
                    ui.group(|ui| {
                        ...
                        ui.group(|ui| {
                            ui.horizontal(|ui| {
                                ...
                                let response_changed = ui
                                    .add(
                                        eframe::egui::TextEdit::singleline(
                                            &mut self.emulated_game.hex_string,
                                        )
                                        .desired_width(60.0)
                                        .hint_text("0000")
                                        .char_limit(4),
                                ...
                                // Parse hex input
                                if let Ok(value) =
                                    u16::from_str_radix(self.emulated_game.hex_string.as_ref(), 16)
                                {
                                    if response_changed {
                                ...
                                ui.add(
                                    eframe::egui::DragValue::new(
                                        &mut self.emulated_game.watched_address_value,
                                    )
                                    .speed(1.0)
                                ...
                                let watch_btn = ui
                                    .add_sized([80.0, 20.0], eframe::egui::Button::new("📌 Watch"));

                                if watch_btn.clicked() {
                                    let is_already_watched = self
                                        .emulated_game
                                        .watched_adress
                                        .addresses_n_values
                                        .iter()
                                        .any(|(address, _)| {
                                            *address == self.emulated_game.watched_address_value
                                        });

                                    if !is_already_watched {
                                        self.emulated_game.watch_address(
                                ...
                                eframe::egui::ScrollArea::vertical()
                                    ...
                                    .show(ui, |ui| {
                                        ...
                                        ui.horizontal(|ui| {
                                            ...
                                            ui.label(
                                                eframe::egui::RichText::new("Address").strong(),
                                            );
                                            ui.label(
                                                eframe::egui::RichText::new("Value (Hex)").strong(),
                                            );
                                            ui.label(
                                                eframe::egui::RichText::new("Value (Dec)").strong(),
                                            );
                                            ui.label(
                                                eframe::egui::RichText::new("Binary").strong(),
                                            );
                                        });

                                        ui.separator();

                                        for (i, (address, value)) in self
                                            .emulated_game
                                            .watched_adress
                                            .addresses_n_values
                                            .iter()
