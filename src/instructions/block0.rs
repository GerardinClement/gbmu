...
use crate::instructions::conditions::COND;
...
fn daa(cpu: &mut CPU) {
	...
	if a == 0 { cpu.registers.set_zero_flag(true); };
...
mod tests {
	...
	fn test_rlca() {
		...
		assert_eq!(cpu.registers.get_carry_flag(), true); // bit 7 = 1
		assert_eq!(cpu.registers.get_zero_flag(), false); // toujours false
		assert_eq!(cpu.pc, 1);
	}
	...
	fn test_rla() {
		...
		assert_eq!(cpu.registers.get_carry_flag(), false); // bit 7 = 0
	...
	fn test_rra() {
		...
		assert_eq!(cpu.registers.get_carry_flag(), false); // bit 0 = 0
		assert_eq!(cpu.registers.get_zero_flag(), false);
		assert_eq!(cpu.pc, 1);
	}
	...
	fn test_daa_addition_no_carry() {
		...
		assert_eq!(cpu.registers.get_zero_flag(), false);
		assert_eq!(cpu.registers.get_half_carry_flag(), false); // cleared
	...
	fn test_daa_addition_with_carry() {
		...
		assert_eq!(cpu.registers.get_zero_flag(), false);
		assert_eq!(cpu.registers.get_carry_flag(), true); // overflow → carry
